...
bool Compile::sc_field_skipped(ciField* f) const {
  if(SCSkipField[0] != '\0'){
    ...
    const char* delim = "::";
    char* name = (char*)calloc(strlen(hname) + strlen(delim) + strlen(fname) + 1, sizeof(char));
    strcpy(name,hname);
    strcat(name,delim);
    strcat(name,fname);
...
bool Compile::sc_loc_skipped(ciField* f) const {
  if(SCSkipLoc[0] != '\0'){
    ...
    const char* delim = "::";
    const char* delim2 = ",";
    const char* hname2 = f->holder()->name()->as_quoted_ascii();
    const char* fname = f->name()->as_quoted_ascii();
    char* name = (char*)calloc(strlen(hname) + strlen(delim) + strlen(mname) +
      strlen(delim2) + strlen (hname2) + strlen(delim) + strlen(fname) + 1, sizeof(char));
    strcpy(name,hname);
    strcat(name,delim);
    strcat(name,mname);
    strcat(name,delim2);
